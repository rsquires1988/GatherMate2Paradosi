...
function Display:MINIMAP_UPDATE_TRACKING()
	if WOW_PROJECT_ID == WOW_PROJECT_CLASSIC then
		...
		local count = (C_Minimap and C_Minimap.GetNumTrackingTypes or GetNumTrackingTypes)();
		local info;
		for id=1, count do
			local name, texture, active, category = (C_Minimap and C_Minimap.GetTrackingInfo or GetTrackingInfo)(id);
